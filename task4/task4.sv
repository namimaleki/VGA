module task4(input logic CLOCK_50,input logic [3:0] KEY,
    ...
    // In task4 we have to do the following:
    // First, we clear the screen
    // Then, draw the Reuleaux triangle centred at (80,60) with diameter 80 and colour green
    ...
    // - DRAW_REULEAUX: run the reuleaux module until it says done
    ...
    // We write one black pixel per clock, scanning y from 0 to 119, then increment x.
    logic [7:0] clr_x;
    logic [6:0] clr_y;
    ...
    // Reuleaux module wires note that the reuleaux module produces its own VGA write signals
    logic reu_done;
    // go flag for reuleaux this will be asserted if we're in the DRAW_REULEAUX state and it will signal the start of the reuleaux module
    logic reu_go;

    logic [7:0] reu_vga_x;
    logic [6:0] reu_vga_y;
    logic [2:0] reu_vga_colour;
    logic reu_vga_plot;

    // Reuleaux module instantiation
    ...
    reuleaux REU(
        ...
        .vga_x(reu_vga_x),
        .vga_y(reu_vga_y),
        .vga_colour(reu_vga_colour),
        .vga_plot(reu_vga_plot)
    );

    // The reuleaux module uses a start/done handshake where start must be held high while drawing, and it will raise done when finished.
    assign reu_go = (top_state == DRAW_REULEAUX);
    ...
    always_comb begin
        ...
        case(top_state)
            ...
            PRE_CLEAR: begin
                ...
                if(all_clear) top_next_state = DRAW_REULEAUX;
            end

            DRAW_REULEAUX: begin
                // Reuleaux module will then run in parallel and we will just wait until it asserts done
                if(reu_done) top_next_state = HOLD_DONE;
            end
    ...
    // VGA drive mux
    // we need a drive mux in place as only one thing can drive the vga ports to framebuffer. So here's how we will figure out wether task4 will drive those inputs or reuleaux:
    // - During clearing states: we drive the VGA core directly with (clr_x, clr_y) and black
    // - During drawing state: we forward the reuleaux module’s VGA outputs
    ...
    always_comb begin
        ...
        if(top_state == BOOT_CLEAR || top_state == PRE_CLEAR) begin
            ...
            VGA_COLOUR = 3'b000;
            VGA_PLOT = 1'b1;
        end else if(top_state == DRAW_REULEAUX) begin
            // reuleaux signals
            VGA_X = reu_vga_x;
            VGA_Y = reu_vga_y;
            VGA_COLOUR = reu_vga_colour;
